...
if (c.includes(oldFunc)) {
  ...
  console.log('OK: Replaced handleSaveWalletId -> handleConnectAsaas');
} else {
  console.log('ERROR: old function not found');
  // Debug
  const idx = c.indexOf('handleSaveWalletId');
  if (idx >= 0) {
...
if (c.includes(oldInputSection)) {
  c = c.replace(oldInputSection, newButtonSection);
  console.log('OK: Replaced input+button section');
} else {
  console.log('WARN: input section not found exactly');
}
...
const newStatus = [
  ...
  "                      ? 'Conta Criada \\u2014 Aguardando Ativa\\u00e7\\u00e3o' ",
  "                      : 'Asaas Conectado'}",
  '                  </span>'
].join('\n');
...
if (c.includes(oldStatus)) {
  ...
  console.log('OK: Updated connected status');
} else {
  console.log('WARN: old status not found');
}
...
if (c.includes(oldWallet)) {
  ...
  console.log('OK: Added pending verification warning');
} else {
  console.log('WARN: wallet section not found');
}

// Write back with LF (Vite handles both)
