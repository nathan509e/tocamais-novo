...
import { 
  ...
  MapPin, Mail, Award, X, ChevronRight, DollarSign
} from 'lucide-react';
    ...
    <AppLayout role="admin">
      <div className="space-y-6 pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className={`text-2xl font-black ${isDark ? 'text-white' : 'text-gray-900'}`}>Painel de Gestão Admin</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total Usuários', value: stats.total, color: isDark ? 'text-white' : 'text-gray-900' },
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className={`w-full pl-10 pr-4 py-2.5 border rounded-xl text-xs focus:outline-none focus:border-neon-purple/50 ${
                isDark 
                  ? 'bg-[#0f0a26] border-white/10 text-white placeholder:text-gray-500' 
          ...
          <select
            ...
            className={`px-4 py-2.5 border rounded-xl text-xs focus:outline-none focus:border-neon-purple/50 ${
              isDark 
                ? 'bg-[#0f0a26] border-white/10 text-white' 
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className={`p-4 rounded-2xl border ${
              ...
              <h3 className={`text-xs font-bold uppercase tracking-wider mb-4 ${isDark ? 'text-white' : 'text-gray-800'}`}>Lista de Contas</h3>
              {loading ? (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <img 
                            src={u.avatar_url || artistProfile?.photo_url || 'https://ui-avatars.com/api/?name=' + u.name + '&background=7B2EFF&color=fff'} 
                            alt="Avatar" 
          ...
          <div>
            {selectedUser ? (
              <div className={`p-5 rounded-2xl border space-y-5 sticky top-24 ${
                isDark ? 'bg-white/5 border-white/5' : 'bg-white border-gray-200 shadow-sm'
              }`}>
                <div className="flex items-start justify-between">
                  <h3 className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-white' : 'text-gray-800'}`}>Ficha do Usuário</h3>
                ...
                <div className="text-center space-y-2 border-b border-white/5 pb-4">
                  <img 
                    src={selectedUser.avatar_url || selectedUser.artistProfile?.photo_url || 'https://ui-avatars.com/api/?name=' + selectedUser.name + '&background=7B2EFF&color=fff'} 
                    alt="Avatar" 
                    className="w-20 h-20 rounded-2xl object-cover mx-auto border-2 border-white/10"
                  />
                  <div>
                    <h4 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>{selectedUser.name}</h4>
                    <p className="text-[10px] text-gray-500 font-mono mt-0.5">{selectedUser.id}</p>
                  </div>
                </div>

                <div className={`space-y-3.5 text-xs ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                  ...
                  {selectedUser.role === 'artist' && selectedUser.artistProfile && (
                    <div className="border-t border-white/5 pt-4 space-y-3">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className={isDark ? 'text-white' : 'text-gray-800'}>{selectedUser.artistProfile.artistic_name}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500 font-bold">Gênero:</span>
                        <span className={isDark ? 'text-white' : 'text-gray-800'}>{selectedUser.artistProfile.genre}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500 font-bold">Cidade:</span>
                        <span className={isDark ? 'text-white' : 'text-gray-800'}>{selectedUser.artistProfile.city}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500 font-bold">Wallet ID Asaas:</span>
                        <span className={`font-mono text-[10px] ${isDark ? 'text-white' : 'text-gray-800'}`}>{selectedUser.artistProfile.asaas_wallet_id || 'Não cadastrado'}</span>
                      </div>
                      ...
                      <div className="space-y-2 pt-2">
                        ...
                        <button
                          onClick={() => navigate(`/admin/orders?search=${encodeURIComponent(selectedUser.artistProfile.artistic_name)}`)}
                          className={`w-full py-2 px-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 border transition-all ${
                            isDark 
                              ? 'bg-white/5 border-white/10 text-gray-300 hover:border-white/20' 
                              : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50'
              ...
              <div className={`p-8 rounded-2xl border text-center text-gray-500 sticky top-24 italic text-xs ${
                isDark ? 'bg-white/5 border-white/5' : 'bg-white border-gray-200 shadow-sm'
              }`}>
