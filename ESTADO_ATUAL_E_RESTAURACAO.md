...
|---|---|---|
| **Produção (arquivos ao vivo)** | `/var/www/tocamais-backup-20260730/` | Cópia completa de `/var/www/tocamais/` feita em 30/07/2026, antes de qualquer alteração |
| **Código-fonte (repositório)** | Git local + GitHub | Commit `85279af` ("att agenda", 27/07/2026) — working tree limpo, sem alterações pendentes |
| **GitHub remoto** | `https://github.com/nathan509e/tocamais-novo` (branch `main`) | `origin/main` está em `31915d8` — **2 commits à frente** do local (ver seção 5) |

**Como restaurar a produção**, se algo der errado:
```bash
rm -rf /var/www/tocamais
cp -a /var/www/tocamais-backup-20260730 /var/www/tocamais
```

**Como restaurar o código-fonte** a este ponto exato, se necessário:
```bash
cd /home/david/tocamais-novo
git checkout 85279af
```
